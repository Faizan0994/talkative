...
exports.deleteUser = async (id) => {
  // Delete all non-group chats the user participates in
  const nonGroupChats = await prisma.chat.findMany({
    where: {
      isGroup: false,
      participants: {
        some: { id: id },
  ...
  if (nonGroupChats.length > 0) {
    await prisma.chat.deleteMany({
      where: {
        id: { in: nonGroupChats.map((c) => c.id) },
      },
    });
  }
...
exports.getChatsForUser = async (userId) => {
  ...
  chats.sort((a, b) => {
    const aTime = a.messages[0]?.timestamp || new Date(0);
    const bTime = b.messages[0]?.timestamp || new Date(0);
...
exports.findChatByName = async (name) => {
...
exports.ensureGroupChat = async (name) => {
  const existing = await exports.findChatByName(name);
...
exports.createMessage = async (userId, chatId, message) => {
  const generatedMessage = await prisma.message.create({
    data: {
      senderId: userId,
      chatId: chatId,
      content: message,
